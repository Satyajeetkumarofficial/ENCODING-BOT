...
async def handle_download_url(message, msg, batch):
    ...
    custom_file_name = sanitize_filename(custom_file_name)
...
def sanitize_filename(name):
    ...
    # Normalize unicode characters to closest ASCII equivalent (e.g. é -> e)
    name = unicodedata.normalize('NFKD', name).encode('ascii', 'ignore').decode('ascii')
    
    # Remove invalid characters and enforce distinct naming (allow alphanumeric, dot, underscore, hyphen)
...
async def handle_tg_down(message, msg, mode='no_reply'):
    ...
    if media and hasattr(media, 'file_name') and media.file_name:
        file_name = sanitize_filename(media.file_name)
    else:
        file_name = ""
