...
async def upload_to_tg(new_file, message, msg):
    # Refresh the message object to avoid FileReferenceExpired errors
    try:
        message = await app.get_messages(message.chat.id, message.id)
    except Exception as e:
        LOGGER.error(f"Failed to refresh message: {e}")
        # Continue with the original message object
