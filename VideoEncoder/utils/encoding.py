...
async def encode(filepath, message, msg, audio_map=None):
    ...
    if m:
        metadata = '-metadata title=ABHI -metadata:s:v title=ABHI -metadata:s:a title=ABHI'
    else:
        metadata = ''
...
#    ])
    ...
    if w:
        ...
        # Copy to temp file to avoid path escaping issues
        if os.path.exists("VideoEncoder/utils/extras/watermark.ass"):
            temp_wm = f"wm_{uuid.uuid4().hex[:8]}.ass"
            shutil.copy("VideoEncoder/utils/extras/watermark.ass", temp_wm)
            watermark += f"subtitles='{temp_wm}'"
        else:
            LOGGER.warning("Watermark file not found!")
    ...
    if not os.path.isfile(output_filepath) or os.path.getsize(output_filepath) == 0:
        LOGGER.error(f"Encoding failed: {output_filepath} not created or empty.")
        return None
